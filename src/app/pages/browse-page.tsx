...
import {
  Search, Filter, BookOpen, User, Calendar, Building, Tag, ChevronRight,
    ...
    <div className="p-6 lg:p-8">
      ...
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">Explore approved engineering research publications</p>
        </div>
      ...
      {!loading && !error && theses.length > 0 && (
        <div className="space-y-4">
          ...
          <div className="space-y-4">
            {theses.map((thesis) => (
              <Card key={thesis.id} className="border-[#2D5016]/10 hover:border-[#4A7C2D]/30 hover:shadow-md transition-all">
                <CardContent className="p-6">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <Link to={`/dashboard/thesis/${thesis.id}`}
                        className="group inline-block">
                        <h3 className="text-xl font-semibold text-[#2D5016] group-hover:text-[#4A7C2D] transition-colors mb-2">
                          {thesis.title}
                        </h3>
                      </Link>
